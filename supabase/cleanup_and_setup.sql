...
DROP TYPE IF EXISTS public.recipe_difficulty CASCADE;
DROP TYPE IF EXISTS public.cuisine_type CASCADE;
DROP TYPE IF EXISTS public.food_allergy CASCADE;
DROP TYPE IF EXISTS public.meal_type CASCADE;

-- Drop functions
DROP FUNCTION IF EXISTS public.update_updated_at_column() CASCADE;
DROP FUNCTION IF EXISTS public.handle_new_user() CASCADE;
DROP FUNCTION IF EXISTS public.reset_meal_credits() CASCADE;

-- Storage buckets: drop policies only, buckets will be recreated with ON CONFLICT
DROP POLICY IF EXISTS "Authenticated users can upload ingredient photos" ON storage.objects;
DROP POLICY IF EXISTS "Anyone can view ingredient photos" ON storage.objects;
DROP POLICY IF EXISTS "Users can delete their own ingredient photos" ON storage.objects;
...
CREATE POLICY "Users can view public recipes or their own"
  ON public.recipes FOR SELECT USING (is_public = true OR created_by = auth.uid());
CREATE POLICY "Authenticated users can insert their own recipes"
  ON public.recipes FOR INSERT TO authenticated WITH CHECK (created_by = auth.uid() OR created_by IS NULL);
CREATE POLICY "Users can update their own recipes"
  ON public.recipes FOR UPDATE TO authenticated USING (created_by = auth.uid()) WITH CHECK (created_by = auth.uid());
...
CREATE POLICY "Users can view their own favorites"
  ON public.favorite_recipes FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own favorites"
  ON public.favorite_recipes FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own favorites"
  ON public.favorite_recipes FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can delete their own favorites"
  ON public.favorite_recipes FOR DELETE USING (auth.uid() = user_id);
...
CREATE POLICY "Users can update their own cooking history"
  ON public.cooking_history FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can delete their own cooking history"
  ON public.cooking_history FOR DELETE USING (auth.uid() = user_id);
...
CREATE POLICY "Users can insert their own stripe customer"
  ON public.stripe_customers FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own stripe customer"
  ON public.stripe_customers FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can delete their own stripe customer"
  ON public.stripe_customers FOR DELETE USING (auth.uid() = user_id);
...
CREATE TRIGGER update_pantry_items_updated_at
  BEFORE UPDATE ON public.pantry_items FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
CREATE TRIGGER update_stripe_customers_updated_at
  BEFORE UPDATE ON public.stripe_customers FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE TRIGGER reset_meal_credits_trigger
  BEFORE UPDATE ON public.profiles FOR EACH ROW EXECUTE FUNCTION public.reset_meal_credits();

-- STORAGE BUCKETS

INSERT INTO storage.buckets (id, name, public) VALUES ('ingredient-photos', 'ingredient-photos', true) ON CONFLICT (id) DO NOTHING;
...
CREATE POLICY "Anyone can view ingredient photos"
  ON storage.objects FOR SELECT USING (bucket_id = 'ingredient-photos');
